...
export default function MotionCanvas({ template, templateVersion }: MotionCanvasProps) {
  ...
  useEffect(() => {
    ...
    const bounds = containerRef.current?.getBoundingClientRect()
    const screenWidth = app.screen?.width || bounds?.width || 1
    const screenHeight = app.screen?.height || bounds?.height || 1
  ...
  }, [template, templateVersion, isReady]) // Re-run when template changes OR when app becomes ready

  return (
    <div className="relative h-full w-full overflow-hidden rounded-lg bg-[#0b0b0b]">
      <div ref={containerRef} className="h-full w-full" />
      {/* Fallback DOM preview so user always sees motion even if Pixi fails */}
      <div
        key={`${template}-${templateVersion}`}
